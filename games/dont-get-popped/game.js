...
// Cold orbs (freezing hazards)
let coldOrbs = [];
const coldOrbSpeed = 2;
const maxColdOrbs = 3;
...
function init() {
    ...
    if (isMobile) {
        ...
        controlsText.textContent = 'Avoid the heat! 🔥 Watch out for ice! ❄️ Drag the kernel or use controls below!';
        setupMobileControls();
    }
...
function handleResize() {
    ...
    // Adjust cold orbs
    coldOrbs.forEach(orb => {
        orb.x = (orb.x / oldWidth) * canvas.width;
        orb.y = (orb.y / oldHeight) * canvas.height;
    });
    ...
    if (wasMobile !== isMobile) {
        ...
        if (isMobile) {
            ...
            controlsText.textContent = 'Use Arrow Keys or WASD to move! Avoid the heat! 🔥 Watch out for ice! ❄️';
        }
    }
}
...
// Create cold orb
function createColdOrb() {
    const x = Math.random() * canvas.width;
    const y = -30;
    const size = 20;
    
    coldOrbs.push({ x, y, size, speed: coldOrbSpeed * gameSpeed });
}
...
// Update cold orbs
function updateColdOrbs() {
    // Move cold orbs
    for (let i = coldOrbs.length - 1; i >= 0; i--) {
        const orb = coldOrbs[i];
        orb.y += orb.speed;
        ...
        if (dist < player.size + orb.size) {
            // Player got frozen!
            playerFrozen = true;
            freezeEndTime = Date.now() + 3000; // Freeze for 3 seconds
            coldOrbs.splice(i, 1);
            continue;
        }
    ...
    // Add new cold orbs
    if (coldOrbs.length < maxColdOrbs && Math.random() < 0.015) {
        createColdOrb();
    }
}
...
function drawPlayer() {
    ...
    // If frozen, add blue tint
    if (playerFrozen) {
        ctx.shadowBlur = 20;
        ctx.shadowColor = '#74b9ff';
    }
    
    ctx.fillStyle = playerFrozen ? '#a7d5ff' : '#f9ca24';
    ctx.strokeStyle = playerFrozen ? '#0984e3' : '#f0932b';
    ctx.lineWidth = 3;
    ...
    // Add detail
    ctx.fillStyle = playerFrozen ? '#0984e3' : '#f0932b';
    ctx.beginPath();
    ...
    if (playerFrozen) {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const x1 = player.x + Math.cos(angle) * 10;
            const y1 = player.y + Math.sin(angle) * 10;
            const x2 = player.x + Math.cos(angle) * 25;
            const y2 = player.y + Math.sin(angle) * 25;
            ctx.beginPath();
            ctx.moveTo(x1, y1);
            ctx.lineTo(x2, y2);
            ctx.stroke();
...
// Draw cold orbs
function drawColdOrbs() {
    coldOrbs.forEach(orb => {
        // Draw cold orb with icy gradient
        const gradient = ctx.createRadialGradient(orb.x, orb.y, 2, orb.x, orb.y, orb.size);
        gradient.addColorStop(0, 'rgba(255, 255, 255, 0.9)'); // White center
        gradient.addColorStop(0.4, 'rgba(116, 185, 255, 0.8)'); // Light blue
        gradient.addColorStop(1, 'rgba(9, 132, 227, 0.7)'); // Dark blue
        ...
        // Add icy crystalline effect
        ctx.strokeStyle = 'rgba(255, 255, 255, 0.8)';
        ctx.lineWidth = 2;
        for (let i = 0; i < 8; i++) {
            const angle = (i * Math.PI) / 4;
            const x1 = orb.x + Math.cos(angle) * 8;
            const y1 = orb.y + Math.sin(angle) * 8;
            const x2 = orb.x + Math.cos(angle) * orb.size;
            const y2 = orb.y + Math.sin(angle) * orb.size;
            ctx.beginPath();
            ctx.moveTo(x1, y1);
            ctx.lineTo(x2, y2);
            ctx.stroke();
